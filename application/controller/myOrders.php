...
class MyOrders extends Controller
{
    ...
    {
        Helper::authenticate();
        Helper::authorize("myOrders/index");

        if (isset($_POST["submit_search_myOrder"])) {
                $tableId = $_POST["tableId"];
            $myOrders = $this->model->getFilteredOrders(
                $_POST["tableId"]
            );
        }
        else{
            $myOrders = $this->model->getAllMyOrders();
            $tableId = '';
        }
       ...
       // load views. within the views we can echo out $orders and $amount_of_orders easily
        ...
        require APP . 'view/myOrders/index.php';
