<div class="menu-container">
        <div class="list-header">
            <div class="title"><i class="fas fa-clipboard-list display-icon"></i>Waiter Order List</div>
                    ...
                    <td class="col-lg-2">
                        <?php
                          if($order->status == OrderStatus::pending){
                            echo '<button class="btn-danger btn-block btn-order-action" data-action="'. OrderStatus::cancelled .'"  data-id="'. $order->id .'"><i class="fa fa-times"></i> Cancel Order</button>';
                          }
                          else if($order->status == OrderStatus::forServing){
                            echo '<button class="btn-success btn-block btn-order-action" data-action="'. OrderStatus::served .'"  data-id="'. $order->id .'"><i class="fa fa-check"></i> Serve</button>';
                          }
                          else if($order->status == OrderStatus::served){
                            echo '<button class="btn-primary btn-block btn-order-action" data-action="'. OrderStatus::forPayment .'"  data-id="'. $order->id .'"><i class="fa fa-check"></i> Ready For Payment</button>';
                          }
                        ?>
                    </td>
                </tr>
